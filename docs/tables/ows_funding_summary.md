# 表契约：silver.ows_funding_summary
...
`report_date` + `entity_code` + `product_category` + `maturity_bucket`。
...
列清单与顺序以 `dbt/models/intermediate/ows_funding_summary.sql` 的最终投影为准；类型由 Spark 在写入 Iceberg 时推断，因此这里只列字段名，不复制一份会过期的类型表。
