# 表契约：silver.owd_loans_history

> 本表是 `silver.owd_loans` 的版本历史，结构 = 基表列 + 6 个版本列（`begin_date`、`end_date`、`is_active`、`last_modified_reason`、`record_version`、`row_hash`）。
...
随基表：`loan_id` 与 `borrower_id` 沿用脱敏 token，本表不含明文。
