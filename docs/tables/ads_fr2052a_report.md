...
**二级资产认列额不单列成列**，按下面的关系取：

```
二级资产认列额 = sec_g_hqla_capped_total_usd − sec_g_hqla_l1_mv
              = min(sec_g_hqla_l2a_mv + sec_g_hqla_l2b_mv, sec_g_hqla_l1_mv × 2/3)
```

两列都是两位小数，相减精确到分，不会掉精度。不单列的理由与代价见 KNOWN-ISSUE 的 `#l2-recognized-not-split`；行级恒等式由 `VDQ-017` 逐行守着，本表不重复断言。

一个例子：`GRP001` 在 2026-09-16 的一级市值 4,434,187,175.29，二级原始市值 4,476,005,271.56，上限 2,956,124,783.53，认列额取上限值，认列总额 7,390,311,958.82。这一行有 15.2 亿二级资产没被认列。
