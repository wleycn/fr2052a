...
`report_id`，四段区位码「机构-报表-报告期-口径」，例 `ENT001-FR2052A-20260916-01`。末段 `01` 为集团并表，`02` 为法人单体。
...
- **上游**：`silver.ows_hqla_summary`、`ows_collateral_summary`、`ows_cash_position`、`ows_cashflow_projection`、`ows_funding_summary` 与各 `owd_*` 明细。
