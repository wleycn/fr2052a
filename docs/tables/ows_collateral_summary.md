# 表契约：silver.ows_collateral_summary
...
担保品汇总：按抵押品类型与 HQLA 分类汇总市值与受限、非受限、可质押金额。
...
`report_date` + `entity_code` + `collateral_type` + `hqla_classification` + `issuer_country`。
...
列清单与顺序以 `dbt/models/intermediate/ows_collateral_summary.sql` 的最终投影为准；类型由 Spark 在写入 Iceberg 时推断，因此这里只列字段名，不复制一份会过期的类型表。
...
- **上游**：`silver.owd_secured_financing`。
- **下游**：报表 Section B 与 Section I。
