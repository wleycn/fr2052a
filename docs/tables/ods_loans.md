...
| `loan_id` | STRING | 贷款编号 |
| `borrower_id` | STRING | 借款人编号 |
| `loan_type` | STRING | 贷款类型原始编码 |
...
| `rate_type` | STRING | 利率类型：FIXED/FLOAT |
| `origination_date` | DATE | 放款日期 |
| `maturity_date` | DATE | 到期日期 |
| `next_payment_date` | DATE | 下次还款日期 |
| `collateral_flag` | STRING | 是否有抵押 Y/N |
| `borrower_type_raw` | STRING | 借款人类型原始值 |
| `credit_grade_raw` | STRING | 信用评级原始值 |
