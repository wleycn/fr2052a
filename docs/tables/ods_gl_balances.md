...
总账余额：科目维度的借贷方余额，用于 Section 级对账。
...
| `gl_account_id` | STRING | 总账科目号 |
| `account_name` | STRING | 科目名称 |
| `debit_balance` | DECIMAL(20,2) | 借方余额 |
| `credit_balance` | DECIMAL(20,2) | 贷方余额 |
...
- **上游**：Kafka 主题 `gl_entries`。
- **下游**：`silver.owd_gl_entries`，再进 `ads_gl_reconciliation` 的对账。
