...
| 2 | 高 | restate.py 的快照哈希两侧序列化不同，「内容未变」短路判据永不成立——每次都登记新版本 | ⬜ 待你定（证据已核，未动） | 需要解释业务逻辑, 没看懂 |
| 3 | 高 | 报送台账唯一键含 file_hash：数据一变就多一行旧台账，verify_submission 之后永久失败 | ⬜ 待你定（证据已核，未动） | 这个是不是应该 使用 report_id 做 跨业务的主键, 而不是 file_hash ?|
...
| 6 | 高 | 报送放行闸只校验熔断闸行存在，不校验本报告日是否跑过预警判定——新报告日可静默放行 | ⬜ 待你定（证据已核，未动） | 这个看起来还是日期处理逻辑的问题, 我建议增加项目级日期控制, 例如: 今天处理的是 哪个 buz_date 的 数据, 这个日期要全系统统一, 可以是T, 也可以是T-1|
| 7 | 中 | 实时扫描丢弃消息自带的报告日，改用命令行常量；与日批每日重放叠加会撞事件表主键 | ✅ 已修（批次 A1）：事件 ID 与落库报告日都改取消息里的值；刻意传错运行参数实测落库报告日仍正确；只清位点重放仍以主键冲突报错退出（重复写入保持显式失败） | ??? |
| 8 | 中 | 三处核对脚本在「一个对象都没核对到」时报告通过（零命中与读不到不可分辨） | ⬜ 待你定（证据已核，未动） | |
| 9 | 中 | 数据质量引擎按异常文本逐表跳过，跳过面不出现在结果里 | ⬜ 待你定（证据已核，未动） | |
| 10 | 中 | 7 条 Kafka 流共用同名临时视图，存在跨表串写风险 | ⬜ 待你定（证据已核，未动） | |
| 11 | 中 | 回执被拒（REJECTED）时生成脚本仍返回 0 | ⬜ 待你定（证据已核，未动） | |
| 12 | 中 | 导出作业的「表结构比对」被宽 except 吞掉，退化成「当作首次导出」 | ⬜ 待你定（证据已核，未动） | |
| 13 | 中 | ADS 层「报表行数 = 交易实体数 + 1」的判据实际只要求 ≥ 2 行 | ⬜ 待你定（证据已核，未动） | |
| 14 | 中 | OWD 折算核对是 inner join，缺汇率的行不进校验却仍报通过 | ⬜ 待你定（证据已核，未动） | |
| 15 | 中 | 重述登记不校验版本区间方向，PG 侧也没有对应约束（与 owd_scd2 的处理不对称） | ⬜ 待你定（证据已核，未动） | |
| 16 | 中 | time_travel 用 f-string 拼表名、快照号与键值，构成注入面 | ⬜ 待你定（证据已核，未动） | |
| 17 | 低 | CSV 导出把布尔列当金额格式化，与 XML 导出同一字段取值不一致 | ⬜ 待你定（证据已核，未动） | |
| 18 | 低 | 明细回溯核对把 NULL 与 0 合并，两侧都为 0 时空集也算通过 | ⬜ 待你定（证据已核，未动） | |
| 19 | 低 | 巡检用 min(submission_status) 代表整批报送状态，REJECTED 会被 ACCEPTED 掩盖 | ⬜ 待你定（证据已核，未动） | |
| 20 | 低 | bronze 层 DDL 注释与入湖实现不符（etl_load_timestamp 的语义已改成数据时间线） | ⬜ 待你定（证据已核，未动） | |
...
| 2 | 高 | GL 对账 Section E 是自比对（两边同读 owd_gl_entries 的 1001/1100），恒 PASS 且零信息量 | ⬜ 待你定（证据已核，未动） |
...
| 19 | 中 | dbt 层零 schema 测试：dbt_project.yml 声明了 test-paths 但目录不存在，marts/schema.yml 只有描述没有断言 | ⬜ 待你定（证据已核，未动） |
...
| 22 | 低 | Section F 的 30 天过滤无下界，已过期贷款也计入流入 | ⬜ 待你定（证据已核，未动） |
| 23 | 低 | 明细表把逆回购也写成 outflow_amount 非零，需依赖 line_item 过滤才与报表对得上 | ⬜ 待你定（证据已核，未动） |
| 24 | 低 | 监管上限的『独立核对』复刻被校验方同一公式，属自证循环 | ⬜ 待你定（证据已核，未动） |
| 25 | 低 | sql/iceberg 下的 05/06/07 是一次性破坏性脚本，却被 deploy/reset-demo.sh 直接引用执行 | ⬜ 待你定（证据已核，未动） |
| 26 | 低 | reset_demo.sql 会 TRUNCATE 被声明为『历史不可变』的报表版本历史表 | ⬜ 待你定（证据已核，未动） |
