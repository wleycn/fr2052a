...
| # | 严重度 | 发现 | 本轮处置 |
|---|---|---|---|
| 1 | 高 | bronze 入湖 MERGE 主键缺 report_date：跨报告日重放会改写上一日快照，且行数核对抓不到 | ⬜ 待你定（证据已核，未动） |
| 2 | 高 | restate.py 的快照哈希两侧序列化不同，「内容未变」短路判据永不成立——每次都登记新版本 | ⬜ 待你定（证据已核，未动） |
| 3 | 高 | 报送台账唯一键含 file_hash：数据一变就多一行旧台账，verify_submission 之后永久失败 | ⬜ 待你定（证据已核，未动） |
...
| 6 | 高 | 报送放行闸只校验熔断闸行存在，不校验本报告日是否跑过预警判定——新报告日可静默放行 | ⬜ 待你定（证据已核，未动） |
| 7 | 中 | 实时扫描丢弃消息自带的 report_date，改用命令行常量；与日批每日重放叠加会撞事件表主键 | ⬜ 待你定（证据已核，未动） |
