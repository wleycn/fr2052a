...
| # | 检查项 | 标准 | 状态 | 证据 |
|---|--------|------|------|------|
| 1 | 命名规范 | 按 CODING-STANDARD.md 逐文件核对 | ⬜ | 本轮只核了改动触及的文件 |
| 2 | 注释质量 | 注释解释「为什么」，出现与实现不符的注释即打回 | ✅ | 本轮修掉 4 处与实现不符的注释（覆盖写机制、熔断触发条件、生效日约定、DQ 日志粒度） |
| 3 | 类型注解 | 新增函数带类型注解 | ✅ | 本轮新增的 4 个脚本全部带注解，写入时由编辑器静态检查通过 |
| 4 | docstring | 公开函数有 docstring，且写明「为什么这么做」 | ✅ | 同上 |
...
| 7 | lint 通过 | — | 🔍 | 不适用：本项目未引入 ruff 配置（无 `pyproject.toml`）。替代：新增脚本交付前由非作者实际调用一次 |
| 8 | mypy 通过 | — | 🔍 | 不适用：未引入类型检查器。替代同第 7 项 |
...
| # | 检查项 | 标准 | 状态 | 证据 |
|---|--------|------|------|------|
| 1 | 端到端重跑 | `bash deploy/reset-demo.sh --apply` 全链路通过 | ✅ | 重置后重跑，16 个环节全绿 |
