...
| # | 检查项 | 标准 | 状态 | 证据 |
|---|--------|------|------|------|
| 1 | 端到端重跑 | `bash deploy/reset-demo.sh --apply` 全链路通过 | ✅ | 复核：**19 个环节全绿**，耗时 **4 分 52 秒**；巡检确认熔断 OPEN、**校验 21 条**、重述 0 次 |
...
| 5 | 变更日志已更新 | `docs/changes/engineering.md` 有对应条目 | ✅ | 逐笔变更落 `docs/changes/engineering.md`；`CHANGELOG.md` 只记里程碑，规则见该文件顶部 |
...
| 6 | 血缘完整 | `LINEAGE.md` 可展示，且血缘表有边 | ✅ | **52 条表级边、36 条列级监管映射**（渲染作业每次整表重建，行数不随重跑累积） |
| 7 | 快照保留与压缩 | 每张 Iceberg 表都有保留策略，且日批自动执行 | ✅ | 日批新增的 `maintain-tables-apply` 环节实测处理 42 张表（ref 9 + bronze 8 + silver 22 + gold 3），逐表设置保留策略并执行小文件合并；过期环节因全部快照都在 7 天窗口内而未删除任何快照 |

## 性能验收

| # | 检查项 | 标准 | 状态 | 证据 |
|---|--------|------|------|------|
| 1 | 全链路时长 | ≤ 15 分钟 | ✅ | 重跑实测 **4 分 52 秒**（19 个环节，含 dbt 19 个模型与 33 条数据测试） |
| 2 | 单环节时长 | 任一环节 ≤ 5 分钟 | ✅ | 同一轮里 19 个环节合计 4 分 52 秒，最慢的环节在 1 分钟内（新增的表维护环节约 40 秒） |
...
> 本项目不做第三方独立复核（见上方「独立复核」行与 `docs/business/KNOWN-ISSUE.md` 的「独立复核不做」行）：签名栏不填写是该项的处置结果，不是漏签。
