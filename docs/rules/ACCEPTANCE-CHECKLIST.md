...
| # | 检查项 | 标准 | 状态 | 证据 |
|---|--------|------|------|------|
| 1 | 端到端重跑 | `bash deploy/reset-demo.sh --apply` 全链路通过 | ✅ | 复核：**18 个环节全绿**，耗时 **3 分 55 秒**；巡检确认熔断 OPEN、**校验 21 条**、重述 0 次 |
...
| 6 | 血缘完整 | `LINEAGE.md` 可展示，且血缘表有边 | ✅ | **49 条表级边、28 条列级监管映射**（渲染作业每次整表重建，行数不随重跑累积） |

## 性能验收

| # | 检查项 | 标准 | 状态 | 证据 |
|---|--------|------|------|------|
| 1 | 全链路时长 | ≤ 15 分钟 | ✅ | 重跑实测 **3 分 55 秒**（18 个环节，含 dbt 18 个模型与 23 条数据测试） |
| 2 | 单环节时长 | 任一环节 ≤ 5 分钟 | ✅ | 同一轮里 18 个环节合计 3 分 55 秒，最慢的环节在 1 分钟内 |
