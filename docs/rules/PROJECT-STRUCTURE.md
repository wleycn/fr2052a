...
| 目录 | 职责 | 谁可写 | 禁止行为 |
|------|------|--------|----------|
| `references/` | 原始需求归档（参考，内容已拆解进 `docs/business/`） | 架构师 | 修改原件（应走变更流程）|
...
| `todo/` | 交接单（会话状态接力，一份一次交接，状态写在文件名后缀 `.new`/`.doing`/`.done`） | 全员 | 改历史交接单的内容（状态迁移用 `git mv` 改名，不删档） |
| `deploy/` | 部署清单（不含凭据） | DevOps | 硬编码密码 |
| `sql/` | DDL（真源） | DBA/架构师 | 直接改线上库 |
| `sql/iceberg/oneoff/` | 一次性破坏性脚本（DROP / 重建 / 批量改数据） | DBA/架构师 | 常规路径（部署脚本、跑批 DAG、reset）引用此目录下的文件 |
