...
| #retired-report-id-in-ledger | 09-19 | 报送台账里 `ENT001-FR2052A-20260916-01` 的 3 行（CSV/XBRL/XML）对应不上任何一条报表：2026-09-16 的报表只有 6 个身份，台账有 7 个 | C3-2a 把「ENT001 兼作合并行」改成「GRP001 承担合并」，报表标识的末段随之分三档。改名之前报出去的那一批在台账里留下 3 行旧身份，状态仍是 ACCEPTED，磁盘上的旧文件也还在 | ⏳ 待裁决：保留当历史留痕，或清掉 3 行台账与磁盘上的旧文件。校验脚本按 report_id 逐条核对，这 3 行与各自文件哈希仍相符，所以 `verify-submission` 不会报红；`pipeline_health` 的报送计数会把这 3 个文件算进去 | 报送台账与报送文件目录 | `docs/changes/engineering.md` 的 submission-ledger-refresh 条目 |
