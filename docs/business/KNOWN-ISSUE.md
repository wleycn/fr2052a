...
| #dangling-ref-source | 09-19 | `ref.ref_regulatory_mapping` 在 `dbt/models/sources.yml` 里声明为数据源，全仓没有一处读取它（`render_lineage.py` 读的是 dbt 元数据，不是这张表）；预警阈值实际来自 `config/liquidity_thresholds.json` | 该字典表原本用来承载监管映射的阈值，实现时阈值改走配置文件，`sources.yml` 的声明没跟着撤 | ✅ 决策：只登记不动代码 —— 表自身由 `ref_data.py` 生成并随 ref 批加载，删声明要另行确认没有外部接入方；表契约与 DATA-DESIGN 已改成实况 | Ref 层 | `docs/tables/ads_fr2052a_alerts.md` |
