...
| 环节 | step | 跑批流水线里可单独调用的一个步骤，有名字与说明 | 可调用环节共 28 个；`run-daily-pipeline.sh --list` 列出默认执行的 19 个 |
...
| 二级资产上限 | HQLA 二级资产（2A+2B）认列额不得超过的比例，基数是扣除后的 HQLA；等价于一级资产市值的 2/3 | 比率 | 6 位小数 | `0.400000` |
...
| 二级资产 | Level 2 Assets | L2A / L2B | 需打折的优质流动性资产，合计认列额不超过一级资产的 2/3（即扣除后 HQLA 的 40%） |
