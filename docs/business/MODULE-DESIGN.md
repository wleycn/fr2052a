...
| 命令 | 参数 | 行为 | 退出码 |
|------|------|------|--------|
| `generate_sample_data.py` | `--out`（默认 `sample_data/`）、`--gl-break-amount`、`--correct-deposit-record`、`--correct-deposit-amount`、`--inject-missing-fx` | 生成 ref/ods CSV，随后自检（含汇率覆盖自检） | 0 全过 / 1 自检不通过 |
