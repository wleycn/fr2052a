...
用途：把建表脚本（sql/iceberg/*.sql）应用到数据湖。之所以单独写一个执行器而不是
直接把 DDL 塞进 Python，是为了让建表语句保持在可读、可 diff、可 Git 管理的 .sql 文件里。
...
限制：按 `--` 剥注释后以分号切分语句。当前建表脚本里没有字符串字面量含分号或 `--`，
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print(f"用法: {argv[0]} <sql 文件路径>", file=sys.stderr)
        return 2
